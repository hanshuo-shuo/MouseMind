...
class Robot(NavigationAgent):
    def __init__(self,
        ...
        # Defaulting to the module preserves upstream behavior for tasks that
        # have not opted into an environment-owned RNG.
        self.rng = rng if rng is not None else random

    def reset(self):
        NavigationAgent.reset(self)
        return AgentState(location=self.rng.choice(self.start_locations), direction=180)
