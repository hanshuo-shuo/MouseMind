"""One-config verified GRPO from the seed-clean SFT planner LoRA."""
...
def make_model(config: Any, base_weight: Path, sft_lora_weight: Path, device: torch.device, *, trainable: bool):
    from model.model_lora import apply_lora, load_lora
    from model.model_minimind import MiniMindForCausalLM

    model = MiniMindForCausalLM(config)
    model.load_state_dict(torch.load(base_weight, map_location="cpu", weights_only=True), strict=True)
    apply_lora(model)
    load_lora(model, sft_lora_weight)
    model.to(device)
    for name, parameter in model.named_parameters():
        parameter.requires_grad_(trainable and ".lora." in name)
    model.eval()  # deterministic likelihoods; gradients remain enabled for LoRA
    return model
...
def main() -> None:
    ...
    policy = make_model(config, args.base_weight, args.sft_lora_weight, device, trainable=True)
    reference = make_model(config, args.base_weight, args.sft_lora_weight, device, trainable=False)
    ...
    weight = args.output_dir / "lora_mousemind_verified_grpo_768.pth"
    save_lora(policy, str(weight))
    os.chmod(weight, 0o600)
    summary = {
        "initial_policy_and_reference": str(args.sft_lora_weight),
        ...
        "mean_kl_to_sft_reference": _mean(all_kl),
